...
class PostProcessWorker:
    """Generate catalog assets from completed TS recordings.

    Proxy HLS and thumbnail generation are stubbed for environments without
    full GStreamer decode support; metadata extraction uses gst-discoverer.
    """
    ...
    def run(self) -> None:
        ...
        try:
            ...
            logger.warning("GStreamer init failed — metadata discovery may be unavailable")
    ...
    def _process(self, job: dict) -> None:
        ...
        with get_session_factory()() as db:
            ...
            master_path = str(segments[0]) if segments else str(master_dir)

            metadata: dict = {"segment_count": len(segments)}
            try:
                metadata.update(discover_file(master_path))
            except Exception:
                logger.warning("Could not discover metadata for %s", master_path)

            asset = Asset(
                recording_id=recording_id,
                title=f"{channel.name if channel else recording.channel_id} — {recording.started_at.isoformat()}",
            ...
            db.add(asset)
            db.commit()
            logger.info("Created asset %s for recording %s", asset.id, recording_id)
